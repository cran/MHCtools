...
#'   in all the output files. These files can be reopened in R using e.g. the
#'   list.load() function in the 'rlist' package.
...
HpltFind <- function(nest_table, seq_table, path_out) {
  ...
  for (i in 1:No_nests) {
    ...
    ### Finally, output a .rdata file with the mean proportion of incongruent
    ...
    list.save(Output, file=paste(path_out, "/Haplotypes_nest", i, "_", c(format(Sys.Date(),"%Y%m%d")), ".rdata", sep=""))
