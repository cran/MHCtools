...
#'   Parent_pairs (created by PapaDiv()) and can be reopened in R using the
#'   list.load() function in the 'rlist' package. For downstream data analysis,
...
PapaDiv <- function(parents_table, seq_table, path_out) {
  ...
  # The .rdata files will be saved in a sub folder in the output path called
  # "Parent_pairs".
  ...
  for (i in 1:length(parents_table$Mother)) {
    ...
    ### Output a .rdata file with the joint diversity of the parent pair, the
    ...
    list.save(Output, file=paste(path_out, "/Parent_pairs/Parent_pair_", i, "_", c(format(Sys.Date(),"%Y%m%d")), ".rdata", sep=""))
