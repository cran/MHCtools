...
#'   files. These files can be reopened in R using e.g. the list.load() function
#'   in the 'rlist' package.
...
ReplMatch <- function(repl_table, seq_table, path_out) {
  ...
  for (i in 1:No_repl) {
    ...
    list.save(Output, file=paste(path_out,"/Repl_set_", i, "_", c(format(Sys.Date(),"%Y%m%d")), ".rdata", sep=""))
